...
(function () {
  ...
  function openPedagogicalConceptModal(conceptType, data) {
    ...
    if (isObjective) {
      if (isServoJoystick) {
        pillars = [
          { icon: 'fa-gamepad', color: '#0D9488', title: 'Lectura Analógica del Joystick (0 a 1023)', desc: 'Adquirir valores continuos de tensión en el pin P0 mediante el convertidor analógico-digital (ADC) de la BBC micro:bit v2.' },
          { icon: 'fa-calculator', color: '#4F46E5', title: 'Ajuste de Intervalo (Mapeo Matemático)', desc: 'Transformar el rango de entrada [0..1023] al rango angular [0°..180°] del servomotor con el bloque matemático mapear.' },
          { icon: 'fa-cogs', color: '#EA580C', title: 'Control Angular PWM del Servomotor SG90', desc: 'Enviar pulsos de modulación por ancho de pulso en el pin P2 para fijar la posición mecánica del eje en tiempo real.' },
          { icon: 'fa-sync-alt', color: '#16A34A', title: 'Bucle Continuo y Estabilización (20 ms)', desc: 'Ejecutar el ciclo de lectura, cálculo y escritura en el bloque "para siempre" con una pausa de 20 ms para evitar vibraciones.' }
        ];
        tipBoxText = '<strong>🕹️ Control de Servo y Joystick con Mapeo Matemático:</strong> En este Nivel 2 de 6° Grado, los estudiantes conectan hardware físico con álgebra lineal. El joystick entrega una señal analógica continua (0 a 1023) y la micro:bit utiliza el bloque matemático de <strong>ajuste de intervalo (mapear)</strong> para calcular el ángulo exacto (0° a 180°) del servomotor SG90.';
      ...
      if (isServoJoystick) {
        pillars = [
          { icon: 'fa-square-root-alt', color: '#4F46E5', title: 'Abstracción Matemática Aplicada', desc: 'Comprender de forma tangible el concepto de proporcionalidad directa y funciones lineales al ver cómo los números orientan un eje físico real.' },
          { icon: 'fa-wave-square', color: '#0D9488', title: 'Señales Analógicas vs. Digitales', desc: 'Diferenciar entre entradas binarias de dos estados (0 o 1) y señales continuas graduadas (valores analógicos de 0 a 1023).' },
          { icon: 'fa-project-diagram', color: '#EA580C', title: 'Integración Hardware-Software', desc: 'Aprender el conexionado de shields de expansión, distribución de alimentación (VCC/GND) y mapeo de pines de entrada y salida.' },
          { icon: 'fa-robot', color: '#16A34A', title: 'Robótica de Control y Servomecanismos', desc: 'Descubrir el principio de funcionamiento de brazos robóticos, timones aeronáuticos y mandos de consolas que traducen gestos en movimiento.' }
        ];
        tipBoxText = '<strong>📐 Matemáticas y Robótica en Acción:</strong> El bloque "ajustar intervalo" (mapear) es el puente entre las matemáticas escolares y la ingeniería robótica. Los estudiantes de 6° grado experimentan el poder del cálculo proporcional para sincronizar el movimiento humano con actuadores mecánicos.';
      } else if (isCanva) {
        pillars = [
                  ...
                  '<div class="apm-slide-grid-2col">' +
                    ...
                    '<div>' +
                      ...
                      '<div class="apm-skills-pills">' +
                        (isServoJoystick ?
                          '<span class="apm-skill-pill" style="background:#CCFBF1;color:#0F766E;"><i class="fas fa-gamepad"></i> Joystick Analógico (P0)</span>' +
                          '<span class="apm-skill-pill" style="background:#CCFBF1;color:#0F766E;"><i class="fas fa-calculator"></i> Ajuste de Intervalo (Mapeo)</span>' +
                          '<span class="apm-skill-pill" style="background:#CCFBF1;color:#0F766E;"><i class="fas fa-cog"></i> Servomotor SG90 (0°-180°)</span>' +
                          '<span class="apm-skill-pill" style="background:#CCFBF1;color:#0F766E;"><i class="fas fa-microchip"></i> BBC micro:bit v2</span>' :
                    ...
                    '<div style="max-width:850px;margin:0 auto;">' +
                      ...
                      '<div class="apm-reto-card" style="margin-top:22px;' + (isServoJoystick ? 'background:#F0FDFA;border-color:#99F6E4;' : (isCanva ? 'background:#F0F9FF;border-color:#BAE6FD;' : (isMarcalibro ? 'background:#FFFBEB;border-color:#FCD34D;' : (isScratchJrPerspectiva ? 'background:#FAF5FF;border-color:#D8B4FE;' : (isScratchJrVelocidad ? 'background:#FFF7ED;border-color:#FDBA74;' : (isDiaMadre ? 'background:#FFF1F2;border-color:#FDA4AF;' : (isPaintBanderas ? 'background:#EFF6FF;border-color:#2563EB;' : (isPaint ? 'background:#F0FDF4;border-color:#16A34A;' : '')))))))) + '">' +
                        ...
                        '<p style="' + (isServoJoystick ? 'color:#134E4A;' : (isMarcalibro ? 'color:#78350F;' : (isScratchJrPerspectiva ? 'color:#6B21A8;' : (isScratchJrVelocidad ? 'color:#7C2D12;' : (isDiaMadre ? 'color:#4C0519;' : (isPaintBanderas ? 'color:#1E3A8A;' : (isPaint ? 'color:#166534;' : ''))))))) + '">' +
                          (isServoJoystick ? '¡El secreto del ajuste de intervalo (mapeo matemático)! El joystick analógico entrega números continuos entre 0 y 1023 según su posición física. Sin embargo, el servomotor solo entiende ángulos de 0° a 180°. Si enviaras 1023 directamente al servo, sobrepasarías su límite mecánico y se trabaría. El bloque matemático <strong>mapear de [0..1023] a [0..180]</strong> aplica la fórmula proporcional: Ángulo = (lectura * 180) / 1023. De este modo, en el centro (512) el servo va a 90°, y a fondo (1023) gira a 180°.' :
                  ...
                  (isServoJoystick ?
                    '<div style="height:100%;display:flex;flex-direction:column;gap:10px;overflow-y:auto;padding-right:6px;">' +
                      '<div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;">' +
                        '<div>' +
                          '<h3 style="font-size:1.2rem;font-weight:900;color:#1E293B;margin:0 0 2px;"><i class="fas fa-gamepad" style="color:#0D9488;"></i> Nivel 2: Control de Servo y Joystick con Mapeo Matemático 🕹️⚙️📐</h3>' +
                          '<p style="font-size:0.82rem;color:#64748B;margin:0;">MakeCode micro:bit • Lectura P0 (0 a 1023) • Ajuste de Intervalo Matemático • Servo P2 (0° a 180°) • Bucle 20 ms</p>' +
                        '</div>' +
                      ...
                      '<div style="display:flex;gap:14px;align-items:center;background:#F0FDFA;border:1.5px dashed #0D9488;border-radius:14px;padding:12px 16px;margin-bottom:6px;">' +
                        '<img src="img/proyectos/servo_joystick_makecode_cover.svg" alt="Servo y Joystick MakeCode" style="width:100px;height:84px;object-fit:cover;background:#0F172A;border-radius:10px;border:1px solid #99F6E4;padding:2px;cursor:pointer;flex-shrink:0;" onclick="window.open(this.src,\'_blank\')" title="Tocar para ampliar esquema">' +
                        '<div style="flex:1;">' +
                          '<div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;flex-wrap:wrap;">' +
                            '<span style="background:#CCFBF1;color:#0F766E;font-weight:800;font-size:0.75rem;padding:2px 8px;border-radius:6px;"><i class="fas fa-project-diagram"></i> CABLEADO HARDWARE</span>' +
                            '<span style="background:#E0E7FF;color:#4338CA;font-weight:800;font-size:0.75rem;padding:2px 8px;border-radius:6px;"><i class="fas fa-calculator"></i> FUNCIÓN: Ángulo = (P0 × 180) / 1023</span>' +
                          '</div>' +
                          '<p style="margin:0;font-size:0.81rem;color:#134E4A;line-height:1.45;">' +
                            '<strong>🕹️ Joystick:</strong> VCC ➔ 3V/5V | GND ➔ GND | VRx ➔ <strong>Pin P0</strong> (lectura analógica 0..1023)<br>' +
                            '<strong>⚙️ Servomotor SG90:</strong> Marrón ➔ GND | Rojo ➔ 3V/5V | Naranja (Señal) ➔ <strong>Pin P2</strong> (ángulo 0°..180°)<br>' +
                            '<strong>📐 Bloque MakeCode:</strong> <code style="background:#E6FFFA;color:#0F766E;padding:1px 6px;border-radius:4px;font-weight:bold;">fijar servo en P2 a: mapear (lectura analógica P0) de: [0..1023] a: [0..180]</code>' +
                          '</p>' +
                        '</div>' +
                      '</div>' +
                      (mkInfo ?
                        '<div class="apm-sim-slide-wrap" style="height:260px;margin-bottom:8px;">' +
                          '<div class="apm-sim-slide-toolbar" style="background:#0F172A;color:#FFF;padding:6px 14px;border-radius:10px 10px 0 0;display:flex;align-items:center;justify-content:space-between;">' +
                            ...
                            '<span style="font-size:0.75rem;color:#94A3B8;">Pin P0 ➔ Entrada Joystick | Pin P2 ➔ Salida Servo</span>' +
                          '</div>' +
                ...
                '<div class="apm-slide-page" data-slide-idx="3">' +
                  '<div style="max-width:850px;margin:0 auto;">' +
                    ...
                    '<div class="apm-extra-challenges">' +
                      (isServoJoystick ?
                        '<div class="apm-ec-item"><div class="apm-ec-badge" style="background:#0D9488;">1</div><div><h6>Invertir la Dirección de Giro</h6><p>Modificá el bloque matemático para mapear de <code>[0..1023]</code> a <code>[180..0°]</code>. ¡Comprobá cómo el joystick ahora mueve el servo en sentido contrario!</p></div></div>' +
                        '<div class="apm-ec-item"><div class="apm-ec-badge" style="background:#0D9488;">2</div><div><h6>Zona Muerta de Reposo (Deadzone)</h6><p>Agregá un condicional <code>si...entonces</code> para que si la lectura está entre 480 y 540 (joystick suelto al centro), el servo se quede fijo exactamente a 90° sin vibrar.</p></div></div>' +
                        '<div class="apm-ec-item"><div class="apm-ec-badge" style="background:#0D9488;">3</div><div><h6>Torreta de Doble Eje (Pan & Tilt)</h6><p>Conectá el eje vertical VRy al pin P1 y un segundo servomotor al pin P8 para controlar un soporte de cámara robótica en dos dimensiones (horizontal y vertical).</p></div></div>' :
                       isMarcalibro ?
             ...
             isElectronica ?
              ...
              (isMakecode ?
                '<div style="height:100%;display:flex;flex-direction:column;">' +
                  (isServoJoystick ? '<div style="background:#F0FDFA;border-left:4px solid #0D9488;padding:10px 14px;border-radius:8px;margin:10px 18px 0;font-size:0.83rem;color:#134E4A;"><strong>🕹️ Circuito Hardware & Mapeo Matemático:</strong> Joystick VRx en <strong>Pin P0</strong> (0..1023) | Servomotor SG90 en <strong>Pin P2</strong> (0..180°) | <strong>Bloque MakeCode:</strong> fijar servo P2 a mapear(lectura analógica P0, de 0..1023 a 0..180°) en bucle <code>para siempre</code> con pausa de 20 ms.</div>' : '') +
